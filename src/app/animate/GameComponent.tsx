...
export default function GameComponent() {
  useEffect(() => {
    ...
    // Fungsi untuk fetch activities, gifts, dan VIP users
    const fetchData = async () => {
      if (!isComponentMounted) return;
      
      try {
        // Fetch activities
        const activitiesResponse = await fetch('http://localhost:5000/api/users/activities');
        const activitiesData = await activitiesResponse.json();
        
        // Fetch gifts
        const giftsResponse = await fetch('http://localhost:5000/api/users/gifts');
        const giftsData = await giftsResponse.json();

        // Fetch VIP users
        const vipResponse = await fetch('http://localhost:5000/api/users/vip');
        const vipData = await vipResponse.json();
        
        if (!isComponentMounted) return;
        
        activities = activitiesData;
        baris1Users = giftsData.slice(0, 14);
        vipUsers = vipData;
        ...
        if (gameScene && gameScene.scene && isComponentMounted) {
          ...
          gameScene.currentBaris1Data = baris1Users;
          gameScene.currentVIPData = vipUsers;
          gameScene.updateBaris2Labels();
          gameScene.updateBaris1Labels();
          gameScene.updateVIPLabels();
        }
        
        if (isComponentMounted) {
          socket.emit('update-baris2', baris2Users);
          socket.emit('update-baris1', baris1Users);
          socket.emit('update-vip', vipUsers);
        }
      } catch (error) {
        if (isComponentMounted) {
    ...
    const initPhaser = async () => {
      ...
      class CharacterScene extends Phaser.Scene {
        ...
        private currentBaris1Data: Baris1User[] = [];
          private currentVIPData: VIPUser[] = [];
          private vipLabels: Phaser.GameObjects.Text[] = [];
        ...
        create() {
          ...
          this.crowds.push(djCrowdLeft);
            ...
            // Create labels for left stage crowd
            djCrowdLeft.labels = this.createStageCrowdLabels(
            this.centerX - 450,
            this.centerY - 170,
              0
          );
            this.vipLabels = [...(djCrowdLeft.labels || [])];
          ...
          this.crowds.push(djCrowdRight);
            ...
            // Create labels for right stage crowd
            djCrowdRight.labels = this.createStageCrowdLabels(
            this.centerX + 450,
            this.centerY - 170,
              7
          );
            this.vipLabels.push(...(djCrowdRight.labels || []));
          ...
          socket.on('update-baris1', (data: Baris1User[]) => {
            ...
            // Socket listener untuk VIP
            socket.on('update-vip', (data: VIPUser[]) => {
              this.currentVIPData = data;
              this.updateVIPLabels();
            });
        ...
        private createCrowdLabels(crowdX: number, crowdY: number, rowPrefix: string, startIndex: number = 0): Phaser.GameObjects.Text[] {
          ...
          private createStageCrowdLabels(crowdX: number, crowdY: number, startIndex: number = 0): Phaser.GameObjects.Text[] {
          const crowdWidth = 256 * this.crowdScale;
          const totalLabelSpacing = crowdWidth * 1.1;
          ...
          for (let i = 1; i <= 7; i++) {
              const userIndex = startIndex + i - 1;
              const user = this.currentVIPData[userIndex];
              let labelText = user ? user.username : `VIP ${i}`;
              
            const personLabel = this.add.text(
              labelStartX + ((i-1) * labelSpacing),
                crowdY - (crowdWidth * 0.31),
                labelText,
              {
                fontFamily: 'Arial',
                fontSize: `${Math.max(Math.min(this.width * 0.008, 12), 8)}px`,
        ...
        private createBaris1Labels(crowdX: number, crowdY: number, startIndex: number): Phaser.GameObjects.Text[] {
          ...
          for (let i = 1; i <= 7; i++) {
            const userIndex = startIndex + i - 1;
              const user = this.currentBaris1Data[userIndex];
              let labelText = user ? user.username : `Baris 1 ${i}`;
        ...
        private updateBaris1Labels() {
            if (!this.baris1Labels) return;
            
          this.baris1Labels.forEach((label, index) => {
              if (!label || !label.setText) return;
              
              const user = this.currentBaris1Data[index];
            if (user) {
        ...
        private updateBaris2Labels() {
          ...
          private updateVIPLabels() {
            if (!this.vipLabels) return;
            
            this.vipLabels.forEach((label, index) => {
              if (!label || !label.setText) return;
              
              const user = this.currentVIPData[index];
              if (user) {
                label.setText(user.username);
              } else {
                label.setText(`VIP ${index + 1}`);
            }
          });
        }
